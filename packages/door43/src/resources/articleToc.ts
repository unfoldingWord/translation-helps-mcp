...
import type { BucketLike, KvLike } from "../platform.js";
...
export interface TocEnv {
  TRANSLATION_HELPS_CACHE?: KvLike;
  ZIP_FILES?: BucketLike;
}
...
async function kvPut(
  ...
  if (!kv) return;
  try {
    await kv.put(key, value, { expirationTtl: TOC_TTL_S });
  } catch {
    /* non-fatal */
  }
}
...
async function buildTaMap(
  ...
  if (listCached) {
    try {
      const articles = JSON.parse(listCached) as Array<{
        path: string;
        title: string;
      }>;
