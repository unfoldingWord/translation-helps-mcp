...
export async function discoverLanguageOrgs(
	...
	try {
		...
		if (!catalogResponse.ok) {
			throw new Error(`Failed to query catalog: ${catalogResponse.status}`);
		}
		...
		const languageMap = new Map<string, { organizations: Set<string>; displayName?: string }>();
			...
			if (!languageMap.has(catalogLangCode)) {
				...
				if (resource.title) {
					...
					if (titleMatch) {
						...
						displayName = displayName.replace(/\s+(ULT|UST|GLT|GST|TN|TW|TQ|TA|TWL)$/i, '');
					}
				}
...
export function getOrganizationsForLanguage(
	...
	const option = discoveryResult.options.find((opt) => opt.language === language);
