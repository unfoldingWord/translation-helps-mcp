...
export class UnifiedResourceFetcher {
	...
	/** Set when a pinned org had no ZIP/catalog matches and results came from all-org retry */
	private organizationFallbackInfo: OrganizationFallbackInfo | undefined;

	constructor(tracer?: EdgeXRayTracer) {
		this.zipFetcher = new ZipResourceFetcher2(tracer);
	}

	getOrganizationFallback(): OrganizationFallbackInfo | undefined {
		return this.organizationFallbackInfo;
	}

	private dedupeScriptureResults(results: ScriptureResult[]): ScriptureResult[] {
	...
	async fetchScripture(
		...
		options?: { _skipOrgFallback?: boolean }
	): Promise<ScriptureResult[]> {
		if (!options?._skipOrgFallback) {
			this.organizationFallbackInfo = undefined;
		}
		...
		if (
			results.length === 0 &&
			!options?._skipOrgFallback &&
			isPinnedSingleOrganization(organization)
		) {
			const requestedOrg = getPinnedOrganizationValue(organization)!;
			logger.info(
				`[fetchScripture] No data for pinned organization=${requestedOrg}, retrying without owner filter`
			);
			const retryResults = await this.fetchScripture(reference, language, undefined, resources, {
				_skipOrgFallback: true
			});
			if (retryResults.length > 0) {
				this.organizationFallbackInfo = makeOrganizationFallbackInfo(requestedOrg);
				return this.dedupeScriptureResults(retryResults);
			}
		}
		...
		if (results.length === 0) {
			...
			try {
				...
				logger.info(
					`No resources found for language=${language}, searching for variants of base=${baseLanguage}`
				);
				...
				const { findLanguageVariants: findVariants } = await import(
					'../../../src/functions/resource-detector.js'
				);
				...
				logger.info(
					`Found ${languageVariants.length} language variants for base=${baseLanguage}:`,
					languageVariants
				);
			} catch (variantError) {
			...
			try {
				...
				if (response.ok) {
					...
					for (const resource of catalogResources) {
						const ingredients =
							resource.ingredients || (resource as any).door43_metadata?.ingredients || [];
			...
			if (languageVariants.length > 0 && availableBooks.length === 0) {
				...
				errorMessage =
					`The book of ${parsed.book} is not available in ${language}.\n\n` +
				...
				const verseInfo = parsed.verse
					? parsed.endVerse
						? `verses ${parsed.verse}-${parsed.endVerse}`
						: `verse ${parsed.verse}`
					: 'this verse';
				...
				errorMessage =
					`Unable to find ${verseInfo} in ${parsed.book} chapter ${parsed.chapter} (${language}).\n\n` +
				...
				if (isValidBookCode) {
					...
					errorMessage =
						`No scripture resources available for language '${language}'.\n\n` +
					...
					errorMessage =
						`Invalid book reference '${requestedBook}'.\n\n` +
		...
		return this.dedupeScriptureResults(results);
	}
	...
	async fetchTranslationQuestions(
		...
		if (!result.data || result.data.length === 0) {
			...
			const languageVariants = await findLanguageVariants(baseLanguage, organization, 'tc-ready', [
