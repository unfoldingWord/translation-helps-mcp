...
async function testMCPEndpoint() {
	...
	try {
		...
		console.log('   Response:', JSON.stringify(initData, null, 2).substring(0, 300));

		if (initResponse.status === 200 && initData.jsonrpc === '2.0' && initData.result) {
	...
	try {
		...
		console.log('   Response preview:', JSON.stringify(toolsData, null, 2).substring(0, 400));

		if (toolsResponse.status === 200 && toolsData.jsonrpc === '2.0' && toolsData.result && toolsData.result.tools) {
			console.log(`   ✅ Tools list successful! Found ${toolsData.result.tools.length} tools\n`);
		} else {
	...
	try {
		...
		console.log('   Response preview:', JSON.stringify(callData, null, 2).substring(0, 400));
	...
	try {
		...
		console.log('   Response:', JSON.stringify(getData, null, 2).substring(0, 300));
