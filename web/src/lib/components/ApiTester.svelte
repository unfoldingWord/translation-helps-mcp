...
<div class="rounded-lg border border-white/10 bg-white/5 p-6">
	...
	{#if endpoint?._lastRequest}
		<div class="mb-4 rounded-lg border border-white/10 bg-black/20">
			<details class="group">
				<summary
				...
				<div class="space-y-4 border-t border-white/10 p-4">
					...
					<div>
						<h4 class="mb-2 flex items-center gap-2 text-xs font-semibold text-gray-400">
							<span class="text-purple-400">🔷</span> MCP Streamable HTTP Request
						</h4>
						<pre
							class="overflow-x-auto rounded bg-gray-900 p-4 text-xs leading-relaxed text-purple-400"><code
								>curl -X POST {window.location.origin}{endpoint._lastRequest.mcpHttp.url} \
  -H "Content-Type: application/json" \
  -H "Accept: application/json" \
  -d '{JSON.stringify(endpoint._lastRequest.mcpHttp.body)}'</code
							></pre>
					</div>
					...
					<div>
						...
						<pre class="overflow-x-auto rounded bg-gray-900 p-4 text-xs text-blue-400"><code
								>curl -X GET "{window.location.origin}{endpoint._lastRequest.rest}"</code
							></pre>
					</div>
					...
					<div>
						...
						<pre class="overflow-x-auto rounded bg-gray-900 p-4 text-xs text-gray-300"><code
								>{JSON.stringify(endpoint._lastRequest.mcp, null, 2)}</code
							></pre>
					</div>
				</div>
			</details>
