...
class RemoteMCPClient {
	...
	async connect(): Promise<void> {
		...
		// Note: Our server supports HTTP POST, but SSE is also compatible
		// For HTTP POST only, you can use fetch() directly (see simple example below)
	...
	async usePrompt(promptName: string, arguments: Record<string, string>): Promise<string> {
		if (!this.client) {
...
| **Authentication** | Usually none           | May require OAuth/API keys (ours is open) |

## HTTP POST vs SSE

Our server supports **HTTP POST** requests. The MCP SDK's `SSEClientTransport` works with our server, but you can also use direct HTTP POST calls:
...
This is simpler than using the full MCP SDK if you just need basic tool calling.
