...
describe('Schema Validation Tests', () => {
  ...
  describe('Required Parameters', () => {
    it('should reject fetch_scripture without reference', async () => {
      ...
      expect(errorThrown, 'Should have thrown error for missing required parameter').toBe(true);
    });
    ...
    it('should reject list_resources_for_language without language', async () => {
      ...
      try {
        ...
        errorThrown = true;
        expect(error.message.toLowerCase()).toMatch(/required|missing|language/);
      }
    ...
    it('should accept valid format values', async () => {
      ...
      for (const format of validFormats) {
        try {
          ...
          if (!error.message.includes('404') && !error.message.includes('not found')) {
            throw new Error(`Valid format '${format}' was rejected: ${error.message}`);
          }
        }
      }
    });
  ...
  describe('Parameter Combinations', () => {
    it('should accept all valid parameter combinations for fetch_scripture', async () => {
      const combinations = [
        ...
        { reference: TEST_DATA.references.singleVerse, organization: 'unfoldingWord' },
        { reference: TEST_DATA.references.singleVerse, language: 'en', organization: 'unfoldingWord' },
      ];
      ...
      for (const params of combinations) {
        try {
          ...
          // 404 is OK
          if (!error.message.includes('404') && !error.message.includes('not found')) {
            throw new Error(`Valid combination ${JSON.stringify(params)} was rejected: ${error.message}`);
          }
        }
      }
    });
  ...
  describe('Tool Schema Consistency', () => {
    it('should have consistent schemas across all tools', async () => {
      ...
      const toolsWithLanguage = tools.filter(t => 
        t.inputSchema?.properties?.language !== undefined
      );

      toolsWithLanguage.forEach(tool => {
        const langSchema = tool.inputSchema.properties.language;
        expect(langSchema.type, `${tool.name} language should be string`).toBe('string');
      });

      const toolsWithOrganization = tools.filter(t => 
        t.inputSchema?.properties?.organization !== undefined
      );

      toolsWithOrganization.forEach(tool => {
        const orgSchema = tool.inputSchema.properties.organization;
        expect(orgSchema.type, `${tool.name} organization should be string`).toBe('string');
      });
    });
  });
});
