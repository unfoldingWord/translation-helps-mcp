...
describe('Error Handling Tests', () => {
  ...
  describe('Missing Required Parameters (400 errors)', () => {
    const testCases = [
      ...
      },
      {
        ...
        params: {},
        expectedPattern: /required.*path/i
      },
      {
  ...
  describe('Invalid Parameter Values (400 errors)', () => {
    it('should reject invalid format with clear error', async () => {
      ...
      try {
        await client.callREST('fetch-translation-word', { path: 'kt/faith', format: 'invalid' });
      } catch (error: any) {
    ...
    it('should reject invalid reference format', async () => {
      ...
      for (const reference of invalidReferences) {
        try {
          ...
          // Error should mention reference or validation
          expect(error.message.toLowerCase()).toMatch(/reference|invalid|not found/);
        }
      }
    });
  });
  ...
  describe('Resource Not Found (404 errors)', () => {
    it('should return 404 for non-existent language', async () => {
      ...
      expect(restError.toLowerCase()).toMatch(/not found|404/);
    });
