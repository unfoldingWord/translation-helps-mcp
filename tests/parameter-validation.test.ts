...
describe("Translation Academy - all-org discovery", () => {
  describe("REST API", () => {
    it("should find es-419 resource (search all owners)", async () => {
    ...
    it("should find resource for es-419 base + variant language", async () => {
      const result = await fetchREST("/api/fetch-translation-academy", {
        path: "translate/figs-metaphor",
        language: "es-419",
        format: "json",
    ...
    it("should find English resource", async () => {
      const result = await fetchREST("/api/fetch-translation-academy", {
        path: "translate/figs-metaphor",
        language: "en",
        format: "json",
  ...
  describe("MCP API", () => {
    it("should find es-419 resource (MCP, all-org)", async () => {
...
describe("Translation Academy - Format Parameter", () => {
  ...
  describe("MCP Format Handling", () => {
    it("should handle md format in MCP and return text", async () => {
      ...
      expect(result.data.content[0].text).not.toBe(
        "No translation academy content found",
      );
    });
...
describe("Translation Academy - Language & Variant Auto-Retry", () => {
  it("should auto-retry es → es-419 (all-org discovery)", async () => {
    const result = await fetchREST("/api/fetch-translation-academy", {
      path: "translate/figs-metaphor",
      language: "es",
      format: "json",
...
describe("Translation Academy - Path Parameter", () => {
  ...
  it("should return TOC when path is omitted", async () => {
    ...
    expect(result.status).toBe(400);
    expect(String(result.data)).toMatch(
      /path|table-of-contents|Translation Academy/i,
    );
  });
...
describe("Translation Word - all-org discovery", () => {
  it("should find Spanish resource", async () => {
  ...
  it("should work with English", async () => {
    const result = await fetchREST("/api/fetch-translation-word", {
      path: "bible/kt/love",
      language: "en",
      format: "json",
...
describe("Cache Behavior (TA)", () => {
  it("should return consistent success for repeated identical TA requests", async () => {
    const params = {
      path: "translate/figs-metaphor",
      language: "es-419",
      format: "json",
    };
    const result1 = await fetchREST("/api/fetch-translation-academy", params);
    expect(result1.status).toBe(200);

    const result2 = await fetchREST("/api/fetch-translation-academy", params);
    expect(result2.status).toBe(200);
    expect((result2.data as { title?: string }).title).toBe(
      (result1.data as { title?: string }).title,
    );
  });
});
