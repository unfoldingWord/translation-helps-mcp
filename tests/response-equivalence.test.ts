...
describe('Response Equivalence Tests', () => {
      ...
      it(`should return equivalent data for ${JSON.stringify(params)}`, async () => {
        ...
        try {
          ...
          if (error.message.includes('404') || error.message.includes('not found')) {
            console.log(`  ⚠️  Resource not found for ${JSON.stringify(params)} (expected for some combinations)`);
            return;
          }
  ...
  describe('Translation Notes Endpoint', () => {
    TEST_COMBINATIONS.translationNotes.slice(0, 2).forEach(params => {
      it(`should return equivalent data for ${JSON.stringify(params)}`, async () => {
        try {
          const restResponse = await client.callREST('fetch-translation-notes', params);
          const mcpResult = await client.callMCPTool({
          ...
          // Verify REST response structure
          expect(restResponse).toBeDefined();
          EXPECTED_STRUCTURES.translationNotes.requiredFields.forEach(field => {
            expect(restResponse).toHaveProperty(field);
          });
          ...
          if (error.message.includes('404') || error.message.includes('not found')) {
            console.log(`  ⚠️  Resource not found for ${JSON.stringify(params)}`);
            return;
          }
          throw error;
        }
  ...
  describe('Translation Word Endpoint', () => {
    TEST_COMBINATIONS.translationWords.forEach(params => {
      it(`should return equivalent data for ${JSON.stringify(params)}`, async () => {
        try {
          const restResponse = await client.callREST('fetch-translation-word', params);
          const mcpResult = await client.callMCPTool({
          ...
          // Verify REST response structure
          expect(restResponse).toBeDefined();
          EXPECTED_STRUCTURES.translationWord.requiredFields.forEach(field => {
            expect(restResponse).toHaveProperty(field);
          });
  ...
  describe('Translation Academy Endpoint', () => {
    TEST_COMBINATIONS.translationAcademy.forEach(params => {
      it(`should return equivalent data for ${JSON.stringify(params)}`, async () => {
        try {
          const restResponse = await client.callREST('fetch-translation-academy', params);
          const mcpResult = await client.callMCPTool({
          ...
          // Verify REST response structure
          expect(restResponse).toBeDefined();
          EXPECTED_STRUCTURES.translationAcademy.requiredFields.forEach(field => {
            expect(restResponse).toHaveProperty(field);
          });
    ...
    TEST_COMBINATIONS.listLanguages.forEach(params => {
      it(`should return equivalent data for ${JSON.stringify(params)}`, async () => {
        ...
        try {
          ...
          expect(mcpObject.languages.length).toBe(restResponse.languages.length);
        } catch {
  ...
  describe('List Resources For Language Endpoint', () => {
    TEST_COMBINATIONS.listResourcesForLanguage.slice(0, 3).forEach(params => {
      it(`should return equivalent data for ${JSON.stringify(params)}`, async () => {
        try {
          const restResponse = await client.callREST('list-resources-for-language', params);
          const mcpResult = await client.callMCPTool({
          ...
          // Verify REST response structure
          expect(restResponse).toBeDefined();
          EXPECTED_STRUCTURES.listResourcesForLanguage.requiredFields.forEach(field => {
            expect(restResponse).toHaveProperty(field);
          });
  ...
  describe('Error Response Consistency', () => {
    ...
    errorTestCases.forEach(({ name, tool, endpoint, params, expectedError }) => {
        ...
        it(`should handle format=${format} consistently for ${tool}`, async () => {
          ...
          try {
            ...
            if (format === 'md' || format === 'markdown') {
              ...
              expect(restResponse.content || typeof restResponse === 'string').toBeTruthy();
