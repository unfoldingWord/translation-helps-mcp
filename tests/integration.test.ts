...
describe('Integration Tests', () => {
  ...
  describe('Organization Filtering Workflow', () => {
    it('should search all organizations when omitted', async () => {
      const response = await client.callMCPTool({
        name: 'fetch_translation_academy',
        arguments: {
          path: TEST_DATA.translationAcademy.metaphor.path,
          language: 'es-419'
          // No organization - should search all
        }
      });
      ...
      console.log('\n  ✅ Search all organizations succeeded');
    }, 10000);
    ...
    it('should filter by specific organization when provided', async () => {
      try {
        ...
        console.log('\n  ✅ Organization filtering succeeded');
      } catch (error: any) {
        // 404 is expected if resource doesn't exist for that org
        if (error.message.includes('404') || error.message.includes('not found')) {
          console.log('\n  ⚠️  Resource not found for specified organization (expected)');
          expect(true).toBe(true);
        } else {
          throw error;
        }
  ...
  describe('Multi-Resource Parallel Fetch', () => {
    ...
    it('should handle mixed success/failure in parallel requests', async () => {
      const results = await Promise.allSettled([
        client.callMCPTool({
          name: 'fetch_scripture',
          arguments: { reference: TEST_DATA.references.singleVerse, language: 'en' }
        }),
        client.callMCPTool({
          name: 'fetch_scripture',
  ...
  describe('End-to-End User Scenarios', () => {
    ...
    it('should support: translator explores new language', async () => {
      ...
      try {
        const scripture = await client.callMCPTool({
          name: 'fetch_scripture',
          arguments: { reference: TEST_DATA.references.singleVerse, language: 'pt-br' }
        });
