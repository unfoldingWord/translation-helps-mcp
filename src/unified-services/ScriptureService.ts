...
} from "../functions/scripture-service.js";
...
> {
  ...
  async execute(
    ...
    try {
      ...
      const options: ScriptureOptions = {
        ...
        ...(params.resource &&
          params.resource !== "all" && {
            specificTranslations: params.resource
              .split(",")
              .map((r) => r.trim()),
          }),
      };
      ...
      if (result.error) {
        throw this.error("SCRIPTURE_ERROR", result.error, result, 404);
      }
      ...
      const scriptureArray =
        result.scriptures || (result.scripture ? [result.scripture] : []);
      const count = scriptureArray.length;
      const resources = scriptureArray
        .map((s: any) => s.translation)
        .filter(Boolean);
      const organizations = scriptureArray
        .map((s: any) => s.citation?.organization)
        .filter(Boolean);
      ...
      return this.success(
        formattedData,
        {
          ...
          ...(result.metadata?.organizationFallback && {
            organizationFallback: result.metadata.organizationFallback,
          }),
        },
        params.format,
      );
      ...
      if (
        error.message?.includes("No scripture resources found") &&
        params.language
      ) {
        try {
          ...
          logger.info(
            `Searching for language variants after resource not found`,
            {
          ...
          if (variants.length > 0) {
            ...
            logger.warn(`No language variants found for base language`, {
              baseLanguage,
            });
          }
        } catch (variantError) {
          // If variant discovery fails, log but don't hide the original error
          logger.error(`Failed to discover language variants`, {
            error: variantError,
          });
        }
      }
   ...
   */
  private formatScriptureResponse(
 ...
 */
export function createScriptureService(
