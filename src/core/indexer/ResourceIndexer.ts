...
// Use crypto.randomUUID() from the Web Crypto API (available in Workers + Node 19+)
import { ZipResourceFetcher2 } from "../resources/ZipResourceFetcher2.js";
...
import type {
  ...
  VectorStoreMetadata,
} from "../rag/interfaces.js";
import type { EmbeddingService } from "../rag/providers/EmbeddingService.js";
  ...
  async indexResource(args: IndexResourceArgs): Promise<IndexResourceResult> {
    ...
    if (!args.zipballUrl) {
      return {
        taskId,
        status: "failed",
        details: `No zipball URL provided for ${args.owner}/${args.project}@${args.refTag}`,
      };
    }
    const zipData = await this.zipFetcher.getOrDownloadZip(args.zipballUrl);
